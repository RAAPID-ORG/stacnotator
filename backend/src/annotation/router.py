...
@router.post(
    "/campaigns/{campaign_id}/ai-segment",
    response_model=AISegmentationResponse,
)
def create_ai_segmentation_annotation(
    campaign_id: int,
    request: AISegmentationRequest,
    db: Session = Depends(get_db),
    user: User = Depends(require_authenticated_user),
    campaign: Campaign = Depends(require_campaign_access),
) -> AISegmentationResponse:
    ...
    """
    result = service.create_annotation_from_ai_segmentation(
        db=db,
        campaign=campaign,
