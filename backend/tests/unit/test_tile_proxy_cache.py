...
from src.imagery.proxy_router import TileTarget
...
def test_the_second_tile_of_a_layer_makes_no_query():
    ...
    def lookup(_db):
        calls.append(1)
        return TileTarget("https://tiles.example/{z}/{x}/{y}?k={api_key}", None)
...
def test_different_layers_do_not_share_an_entry():
    def lookup_for(url):
        return lambda _db: TileTarget(url, None)
...
def test_a_campaign_cannot_read_another_campaigns_cached_target():
    ...
    a = _resolve(("basemap", 1, 42), lambda _db: TileTarget("https://one.example/{z}", None))
    b = _resolve(("basemap", 2, 42), lambda _db: TileTarget("https://two.example/{z}", None))
    assert a.url == "https://one.example/{z}"
    assert b.url == "https://two.example/{z}"
...
def test_an_entry_is_dropped_once_its_ttl_passes(monkeypatch):
    ...
    def lookup(_db):
        calls.append(1)
        return TileTarget("https://tiles.example/{z}", None)
...
def test_a_missing_layer_is_not_cached():
    ...
    with pytest.raises(HTTPException):
        _resolve(("basemap", 1, 42), lookup)
    state["exists"] = True
    assert _resolve(("basemap", 1, 42), lookup).url == "https://tiles.example/{z}"
...
def test_the_cache_is_bounded(monkeypatch):
    ...
    for i in range(40):
        _resolve(("basemap", 1, i), lambda _db, n=i: TileTarget(f"https://tiles.example/{n}", None))
...
def test_the_db_slot_is_released_before_the_provider_fetch():
    ...
    async def scenario():
        await proxy_router._resolve(
            ("basemap", 1, 42), lambda _db: TileTarget("https://x/{z}", None)
        )
