...
def test_category_with_non_int_value_raises():
    with pytest.raises(ValueError, match="Crop Type"):
        format_form_value(FIELDS[0], "Wheat")


def test_multicategory_with_non_list_value_raises():
    with pytest.raises(ValueError, match="Damage"):
        format_form_value(FIELDS[1], 1)


def test_multicategory_with_mixed_list_raises():
    with pytest.raises(ValueError, match="Damage"):
        format_form_value(FIELDS[1], [1, "Drought"])


def test_daterange_with_plain_string_raises():
    with pytest.raises(ValueError, match="Season"):
        format_form_value(FIELDS[3], "2026-05-01")


def test_daterange_with_incomplete_dict_raises():
    with pytest.raises(ValueError, match="Season"):
        format_form_value(FIELDS[3], {"start": "2026-05-01"})
