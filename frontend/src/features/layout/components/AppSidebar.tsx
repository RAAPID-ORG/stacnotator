...
export const AppSidebar = ({
  ...
  // On mobile (<sm), sidebar is an off-canvas drawer driven by mobileOpen;
  // on desktop it's the inline collapsible aside. The wrapper handles both
  // modes via responsive classes so we don't duplicate the markup.
  return (
    <>
      {mobileOpen && (
        <div
          onClick={() => setMobileOpen?.(false)}
          className="desktop:hidden fixed inset-0 bg-black/30 z-40"
          aria-hidden
        />
      )}
      <aside
        className={`flex flex-col h-full bg-white border-r border-neutral-200 fixed desktop:static inset-y-0 left-0 z-50 transform transition-transform desktop:transform-none ${
