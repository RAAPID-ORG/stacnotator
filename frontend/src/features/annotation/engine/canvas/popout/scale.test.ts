...
import { withoutKeys, mergeLayoutChange, packItem, scaleWidthToScreen } from './scale';
...
describe('packItem', () => {
  ...
  it('starts a fresh row when no slot fits', () => {
    const screen: LayoutItem[] = [{ i: 'a', x: 0, y: 0, w: 60, h: 10 }];
  ...
  it('is a no-op when the card is already on the screen', () => {
    const screen: LayoutItem[] = [{ i: 'controls', x: 5, y: 5, w: 20, h: 12 }];
...
describe('mergeLayoutChange', () => {
  ...
  it('passes the layout through untouched when nothing is popped', () => {
    const next: LayoutItem[] = [{ i: 'main', x: 0, y: 0, w: 60, h: 20 }];
