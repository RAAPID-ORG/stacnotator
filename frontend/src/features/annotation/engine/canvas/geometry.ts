import { GRID_COLS, GRID_MARGIN, ROW_HEIGHT, type LayoutItem } from './types';
...
/** Compute where to drop a freshly-added item so that adding items one after
 ...
 *  platform/canvas has no notion of fixed layout keys, so an empty layout
 *  simply starts at the origin. */
export function nextFreeSlot(layout: LayoutItem[], w: number, h: number): Point {
