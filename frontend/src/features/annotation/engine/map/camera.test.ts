...
describe('camera state', () => {
  it('round-trips lon/lat through the mercator view', () => {
    const camera = createCamera({ center: [12.34, 56.78], zoom: 9 });
  ...
  it('moves center and zoom independently', () => {
    const camera = createCamera({ center: [0, 0], zoom: 4 });
  ...
  it('pans by screen pixels: +x is east, +y is south', () => {
    const camera = createCamera({ center: [0, 0], zoom: 8 });
...
describe('getBounds', () => {
  it('matches the bounds an onChange snapshot carries, synchronously', () => {
    const camera = createCamera({ center: [10, 45], zoom: 6 });
...
describe('lonLatFromContainerPixel', () => {
  it('returns the camera center for the container center', () => {
    const camera = createCamera({ center: [12, 34], zoom: 6 });
  ...
  it('moves east for a pixel right of center, and north for a pixel above it', () => {
    const camera = createCamera({ center: [0, 0], zoom: 6 });
  ...
  it('scales with resolution: the same pixel offset covers more ground when zoomed out', () => {
    const zoomedIn = createCamera({ center: [0, 0], zoom: 10 });
    const zoomedOut = createCamera({ center: [0, 0], zoom: 4 });
...
describe('follow', () => {
  it('mirrors the leader until unfollowed', async () => {
    const leader = createCamera({ center: [0, 0], zoom: 4 });
    const follower = createCamera({ center: [0, 0], zoom: 4 });
  ...
  it('snaps to the leader position immediately on follow', () => {
    const leader = createCamera({ center: [10, 20], zoom: 7 });
    const follower = createCamera({ center: [0, 0], zoom: 2 });
