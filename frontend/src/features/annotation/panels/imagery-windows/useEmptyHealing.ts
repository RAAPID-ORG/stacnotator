...
import { refreshTilerSession, type LonLat } from '~/features/annotation/engine/map';
...
async function probeEmpty(target: TileTarget, signal: AbortSignal): Promise<boolean> {
  if (target.credentialed) await refreshTilerSession();
...
export function useEmptyHealing(args: UseEmptyHealingArgs): EmptyHealingResult {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const i of probe.emptyIndices) markEmptyRef.current(emptyKey(address.collectionId, i));
