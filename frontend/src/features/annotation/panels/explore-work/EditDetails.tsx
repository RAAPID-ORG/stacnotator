...
import type { FormValues } from '~/features/annotation/core/apiTypes';
...
function DetailsForm({ ctx, annotation }: { ctx: ComposeCtx; annotation: AnnotationOut }) {
  ...
  const save = async () => {
    ...
    try {
      const result = await updateAnnotationOpenmode({
        path: { campaign_id: ctx.campaign.id, annotation_id: annotation.id },
        body: {
          label_id: labelId,
      ...
      setEditAnnotation(result.data ?? { ...annotation, label_id: labelId, form_values: values });
        ...
        <textarea
          ...
          onBlur={() => {
            if ((annotation.flag_comment ?? '') !== flagComment) {
              void saveAnnotationFlag(ctx.campaign.id, true, flagComment || null);
            }
          }}
...
export function EditDetails({ ctx }: EditDetailsProps) {
  const selection = useWorkStore((s) => s.selection);
  const annotation = useEditSession((s) => s.annotation);
  ...
  return <DetailsForm key={annotation.id} ctx={ctx} annotation={annotation} />;
}
