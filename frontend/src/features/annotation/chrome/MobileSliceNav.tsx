...
export function MobileSliceNav() {
  ...
  const collections = collectionsInView(catalog, view);
  const sliceCount = address
    ? (catalog.collections.get(address.collectionId)?.slices.length ?? 0)
    : 0;
  ...
  return (
    ...
    >
      {hasMultipleCollections && (
        <NavButton onClick={handle(() => stepCollection(catalog, -1))} label="Previous collection">
          <IconChevronDoubleLeft className="h-5 w-5" />
        </NavButton>
      )}
      {hasMultipleSlices && (
        <NavButton onClick={handle(() => stepSlice(catalog, -1))} label="Previous slice">
          <IconChevronLeft className="h-5 w-5" />
        </NavButton>
      )}
      {hasMultipleSlices && (
        <NavButton onClick={handle(() => stepSlice(catalog, 1))} label="Next slice">
          <IconChevronRight className="h-5 w-5" />
        </NavButton>
      )}
      {hasMultipleCollections && (
        <NavButton onClick={handle(() => stepCollection(catalog, 1))} label="Next collection">
