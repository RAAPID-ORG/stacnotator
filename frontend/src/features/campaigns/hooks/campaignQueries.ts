...
import type { AnnotationOut, AnnotationTaskOut, TaskSetOut } from '~/api/client';
import {
...
export const useCampaignAnnotations = (campaignId: number) => {
  const { data, isPending } = useQuery({
    ...getAllAnnotationsForCampaignOptions({ path: { campaign_id: campaignId } }),
    ...SHARED_WORK,
    meta: { errorMessage: 'Failed to load annotations' },
  });
  return { annotations: data ?? NO_ANNOTATIONS, loading: isPending };
};

/** The agreement figures. The panel shows its own failure line, so no toast. */
export const useCampaignStatistics = (campaignId: number) => {
