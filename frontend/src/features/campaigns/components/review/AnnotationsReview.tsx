...
import {
  ...
  getAnnotationFacetsQueryKey,
  listAnnotationsForCampaignQueryKey,
} from '~/api/queries';
import { useAnnotationFacets, useAnnotationsPage } from '../../hooks/campaignQueries';
...
export const AnnotationsReview = ({
  campaign,
  campaignId,
  headerActions,
  subHeader,
}: AnnotationsReviewProps) => {
  ...
  // Narrowing a filter can leave the reader on an offset past the end of its own
  // results, so any change to what is being asked for returns them to the first page.
  ...
  const facets = useAnnotationFacets(campaignId);
  const { items, total, loading, refreshing } = useAnnotationsPage(campaignId, {
    limit: PAGE_SIZE,
    offset,
    ...filters,
  });
  ...
  // Deleting changes both the page and the campaign-wide counts, so both are
  // invalidated rather than patched by hand: the server decides what is left.
    ...
    onSuccess: async (result) => {
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: listAnnotationsForCampaignQueryKey({ path: { campaign_id: campaignId } }),
        }),
        queryClient.invalidateQueries({
          queryKey: getAnnotationFacetsQueryKey({ path: { campaign_id: campaignId } }),
        }),
      ]);
  ...
  const handleBatchDelete = () => {
    ...
    batchDelete.mutate({
      ...
      body: { annotation_ids: Array.from(selectedAnnotationIds) },
    });
  };
            ...
            {deletablePageIds.length > 0 && (
              <div className="flex items-center justify-between gap-3 px-4 py-2 border-b border-neutral-200 bg-neutral-50">
                ...
                <Button
                  ...
                  disabled={selectedAnnotationIds.size === 0 || batchDelete.isPending}
                >
