...
export const StepImagery = ({
  ...
  const syncToForm = (s: ImageryStepState) => {
    const imageryConfigs = s.views.flatMap((view) =>
      view.collectionRefs
        .flatMap((ref) => {
          const resolved = resolveCollection(s.sources, ref);
          if (!resolved) return [];
          const { collection, source } = resolved;
          const data = collection.data;
          return collection.slices.map((slice) => ({
            name: slice.name || collection.name || `${slice.startDate} - ${slice.endDate}`,
            start_ym: slice.startDate.slice(0, 7).replace('-', ''),
            end_ym: slice.endDate.slice(0, 7).replace('-', ''),
            crosshair_hex6: source.crosshairHex6,
            default_zoom: source.defaultZoom,
            window_interval: collection.windowInterval ?? null,
            window_unit: collection.windowUnit ?? null,
            slicing_interval: collection.slicingInterval ?? null,
            slicing_unit: collection.slicingUnit ?? null,
            registration_url: data.type === 'stac' ? data.registrationUrl : '',
            search_body: data.type === 'stac' ? data.searchBody : '',
            visualization_url_templates: data.vizUrls.map((v) => ({
              name: v.vizName,
              visualization_url: v.url,
            })),
          }));
        }),
    );
    ...
    setForm({
      ...form,
      imagery_configs: imageryConfigs.length > 0 ? imageryConfigs : null,
    });
  };
