...
- **Drawn in the browser** - the owner keeps the campaign's Agents page (`.../campaigns/<id>/agents`) open. It claims render jobs, draws them with the same tiles and layers as the annotation page, and uploads the result. The server never renders imagery.
- **Prerendering** - the views an agent requests for its current task are also queued for its next two tasks, so a steady workflow gets its bundle without waiting.
